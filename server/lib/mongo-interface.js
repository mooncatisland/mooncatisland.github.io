...
export default class MongoInterface  {
   ...
   async findOneAndUpdate(collectionName,query,newvalues)
    ...
    //MAKE THIS WORK
    async findWithJoin(collectionName, joinParams ){

      let params = [{
        $lookup:{
          from: joinParams.otherCollection,
          localField: joinParams.localField,
          foreignField: joinParams.foreignField,
          as: joinParams.as
      ...
      return new Promise(function(resolve, reject) {
          this.dbo.collection(collectionName).aggregate(joinParams, function(err, res) {
            if (err) reject(err);
          //  console.log("1 inserted ",collectionName);
            resolve(res);
          });
